...
function ModalCadastroUsuario({ isOpen, onClose, usuario }: ModalCadastroUsuarioProps) {
  ...
  // Observar tipo de usuário para mostrar/esconder campos
  const tipoUsuario = watch('tipo');
  const watchedEspecialidades = watch('especialidades');
  const isMedico = Number(tipoUsuario) === 2;

  // Carregar dados do usuário para edição
  useEffect(() => {
    if (isEditing && usuario && isOpen) {
      // Agrupar especialidades por especialidadeId (apenas para médicos)
      let especialidadesAgrupadas: MedicoEspecialidade[] = [];
      
      if (usuario.tipo === 2 && usuario.especialidades) {
        const agrupamento = usuario.especialidades.reduce((acc, item) => {
        ...
        especialidadesAgrupadas = Object.values(agrupamento);
      }
      ...
      reset({
        ...
        tipo: usuario.tipo,
        crm: usuario.crm || '',
        especialidades: usuario.tipo === 2 && especialidadesAgrupadas.length > 0 
          ? especialidadesAgrupadas 
          : [{ especialidadeId: 0, convenioIds: [], tempoConsulta: 30 }],
      });
  ...
  }, [isEditing, usuario, isOpen, reset]);
  ...
  const onSubmit = async (data: UsuarioFormData) => {
    ...
    const tipoNum = Number(data.tipo);
    ...
    try {
      ...
      if (isEditing) {
        ...
        if (!data.senha) {
          ...
          const { senha: _, ...payloadSemSenha } = payload as CreateUsuarioRequest & { senha?: string };
          await updateUsuario({
            id: usuario.id,
            data: payloadSemSenha as unknown as UpdateUsuarioRequest
          }).unwrap();
        } else {
          await updateUsuario({
            id: usuario.id,
            data: payload as unknown as UpdateUsuarioRequest
          }).unwrap();
        }
        toast.success('Usuário atualizado com sucesso!');
      } else {
        await createUsuario(payload as CreateUsuarioRequest).unwrap();
      ...
      const errorMessage = (error as { data?: { message?: string } })?.data?.message || 'Erro ao salvar usuário';
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="form-control">
            <label className="label">
              <span className="label-text font-semibold text-base">Tipo de Usuário *</span>
            </label>
            <select
              className={`select select-bordered select-primary ${errors.tipo ? 'select-error' : ''}`}
              {...register('tipo', { required: 'Tipo é obrigatório' })}
              disabled={isEditing} // Não permite mudança de tipo na edição
            >
            ...
            {isEditing && (
              <label className="label">
                <span className="label-text-alt text-info">O tipo de usuário não pode ser alterado após a criação</span>
              </label>
            )}
            {errors.tipo && (
              <label className="label">
                <span className="label-text-alt text-error">{errors.tipo.message}</span>
              </label>
            )}
          </div>

          {/* ========== DADOS BÁSICOS ========== */}
          <div className="divider">Dados Básicos</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="form-control">
              ...
              {isEditing && (
                <label className="label">
                  <span className="label-text-alt">Deixe em branco para manter a senha atual</span>
                </label>
              )}
            </div>
          </div>
          ...
          {isMedico && (
            <>
              <div className="divider">Dados do Médico</div>
              ...
              <div className="form-control">
                ...
                <input
                  ...
                  {...register('crm', { 
                    required: isMedico ? 'CRM é obrigatório para médicos' : false 
                  })}
                />
                {errors.crm && (
                  <label className="label">
                      ...
                      >
                        <span className="font-medium">
                          ...
                          {(() => {
                            ...
                            const especialidade = (especialidadeId && especialidadeId > 0) ? todasEspecialidades.find(e => e.id === especialidadeId) : null;
